...
- Every card shows a small **date badge** top-center, above the title — the visit's `Initial Home Visit Date`, formatted like "Aug 17" (`formatMonthDay()` in `site.js`).
- **Card layout is spill-proof:** `.card` has `overflow: hidden`, and the diagonal corner "stamp" (OPEN/COVERED/CLAIMED) is built by `makeStamp()` and appended to the card-wrap *outside* the card element — not inside it — so it can still overhang the corner on purpose while nothing else (the status oval, date, etc.) can ever visually spill onto the blue page background. The status oval itself is also white-filled with a colored border/text (not a solid color fill), so it reads clearly against either backdrop.
