...
**Monthly rollover** (unchanged in spirit): a need Covered in a *prior* month disappears from the board automatically; Open/Partially Covered needs keep showing regardless of age; a same-month Covered win still shows before it rolls off. This uses each expanded need's `month_posted`, taken straight from the workbook's calculated `Month Posted` column. A **Closed** need (a request that couldn't be met) never shows anywhere — not the board, not the archive.

The **"Families We've Served" archive** (`servedNeeds()` in `site.js`, collapsed by default under the live board) is every need marked Covered within a **rolling 12-month window** — it's what a Covered need rolls off into once its covered-month passes, and it rolls back OUT of the archive entirely once it's more than 12 months old. This is a deliberately different scope than the glance card's YTD figures: the glance card resets every January (a calendar-year question, "how are we doing this year"), while the archive always shows "the past 12 months as of right now" (a rolling question, "who have we helped recently") — so the two numbers won't match, and that's by design, not a bug. The summary line counts distinct **families**, not needs — a family with e.g. both a rent need and a utility need covered is one family, not two, deduped the same way as `outstandingNeedsSummary()`'s families count elsewhere in `site.js` (via the shared `<visitid>-X` id prefix). The archive reuses the exact same card renderers as the live board (`renderStatusCard()` / `renderClaimCard()` / `renderWarehouseCard()`), so archive cards show the same home-visit date + status badge format, just with a sage-green thumbtack instead of red as the one visual cue that they're past, resolved needs. There's no separate "explain how this works" copy on the board anymore — that legend card (Furnishings vs. Rent/Utility, warehouse vs. parish fund) was removed along with the money-focused framing; the board is just the cards now.
