...
- Every card shows a solid-color **status badge** (Open = rust/orange, Partially Covered = gold, Covered/Claimed = sage/green — `statusColor()` in `site.js`) inside the card, top-left — this is the only status indicator; there's no diagonal "stamp" anymore, to avoid showing status twice on the same card.
- The **date badge** sits top-right, inside the card, next to the status badge's row.
